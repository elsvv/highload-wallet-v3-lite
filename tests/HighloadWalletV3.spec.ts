...
describe('HighloadWalletV3', () => {
  ...
  beforeAll(async () => {
    ...
    getContractData = async (address: Address) => {
      ...
      if (smc.account.account.storage.state.type != 'active') throw 'Atempting to get data on inactive account';
    ...
    getContractCode = async (address: Address) => {
      const smc = await blockchain.getContract(address);
      if (!smc.account.account) throw 'Account not found';
      if (smc.account.account.storage.state.type != 'active') throw 'Atempting to get code on inactive account';
  ...
  it('should pass check sign', async () => {
    try {
      ...
      const testResult = await highloadWalletV3.sendExternalMessage(keyPair.secretKey, {
        createdAt: 1000,
        query_id: queryId,
        message,
        mode: 128,
  ...
  it('should fail check sign', async () => {
    ...
    await shouldRejectWith(
      highloadWalletV3.sendExternalMessage(badKey, {
        createdAt: 1000,
        query_id: queryId,
        message,
        mode: 128,
  ...
  it('should fail subwallet check', async () => {
    ...
    expect(curSubwallet).toEqual(HWV3_SUBWALLET_ID);
    ...
    await shouldRejectWith(
      highloadWalletV3.sendExternalMessage(keyPair.secretKey, {
        createdAt: 1000,
        query_id: queryId,
        mode: 128,
        message,
  ...
  it('should fail check created time', async () => {
    ...
    await shouldRejectWith(
      ...
      HWV3Errors.invalid_creation_time,
    );
  });
  ...
  it('should be cleared queries hashmaps', async () => {
    ...
    const testResult1 = await highloadWalletV3.sendExternalMessage(keyPair.secretKey, {
      createdAt: 1000,
      query_id: queryId,
      message,
      mode: 128,
    ...
    const testResult2 = await highloadWalletV3.sendExternalMessage(keyPair.secretKey, {
      createdAt: 1200,
      query_id: newQueryId,
      message,
      mode: 128,
  ...
  it('queries dictionary with max keys should fit in credit limit', async () => {
    ...
    const queryId = HighloadQueryId.fromShiftAndBitNumber(BigInt(rndShift), BigInt(rndBitNum));
    const res = highloadWalletV3.sendExternalMessage(keyPair.secretKey, {
      createdAt: 1000,
      query_id: queryId,
      message,
      mode: 128,
  ...
  it('should send internal message', async () => {
    ...
    let res = await highloadWalletV3.sendExternalMessage(keyPair.secretKey, {
      query_id: queryId,
      message: internal_relaxed({
        to: testAddr,
        bounce: false,
